...
function renderStoryMap() {
    ...
    // Melhoria: Não mostramos a intro da jornada se o usuário for novo e ainda não resgatou boas-vindas
    // (Isso será controlado pelo script.js chamando initStoryMode após o onboarding)
    ...
    jornada.chapters.forEach((chapter, chapterIdx) => {
        ...
        chapterEl.className = `story-chapter ${isGuest ? 'locked-for-guest' : ''}`;
        
        // Se for visitante, o capítulo inteiro fica com um overlay ou comportamento de bloqueio
        if (isGuest) {
            chapterEl.onclick = (e) => {
                e.stopPropagation();
                if (typeof openConversionModal === 'function') openConversionModal();
            };
        }
